...
npm run check
git add .
git commit -m "Release 0.9.8"
git push
git tag v0.9.8
git push origin v0.9.8
```

После успешной GitHub Actions сборки скачай артефакт `NightVault-Windows` и создай GitHub Release с тегом `v0.9.8`. В релиз обязательно прикрепить:

- `NightVault-Setup-0.9.8.exe`
- `latest.yml`
