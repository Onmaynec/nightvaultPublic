...
const nv = window.nv || {
  ...
  getVersion:async()=> 'dev', checkUpdates:async()=>({dev:true}), downloadUpdate:async()=>({ok:false}), installUpdate:async()=>({ok:false}),
...
const defaultState=()=>({
  ...
  theme:localStorage.nvTheme||'crimson', accent:localStorage.nvAccent||'#e11b2f', blur:localStorage.nvBlur==='1',
  selected:new Set(), editId:null, micId:localStorage.nvMicId||'', accounts:JSON.parse(localStorage.nvAccounts||'[]'), chatBg:localStorage.nvChatBg||'particles', fontSize:Number(localStorage.nvFontSize||15), fontFamily:localStorage.nvFontFamily||'system', notes:localStorage.nvNotes||''
});
let S=defaultState();
let sock=null;
...
async function api(path,opt={}){
  const r=await fetch(getServerApi()+path,{...opt,headers:{'Content-Type':'application/json',Authorization:'Bearer '+S.token,...(opt.headers||{})}});
...
function titlebar(){return S.fullscreen?'':`<div class="titlebar"><b>Night<span>Vault</span></b><div class="winBtns"><button id="winMin">—</button><button id="winFull">▢</button><button id="winClose">×</button></div></div>`}
...
async function init(){
  ...
  if(S.token){try{let r=await api('/me');S.user=r.user;S.settings=r.settings||{};connect();await loadChats(false);render();setTimeout(()=>scrollChatBottom(false),0);return}catch{localStorage.removeItem('nvToken');S.token=''}}
  renderAuth();
}
function renderAuth(){
...
function connect(){
  ...
  sock=new WebSocket(getServerWs()+'?token='+S.token);
...
function render(){if(!S.user)return renderAuth();applyVisualPrefs();app.innerHTML=titlebar()+`<div class="shell ${S.fullscreen?'fullscreen':'windowed'}"><div class="tabs"><button class="tab ${S.tab==='chats'?'active':''}" onclick="S.tab='chats';render()">💬<small>Чаты</small>${S.chats.reduce((a,c)=>a+(c.unread||0),0)?'<span class=count>'+S.chats.reduce((a,c)=>a+(c.unread||0),0)+'</span>':''}</button><button class="tab ${S.tab==='contacts'?'active':''}" onclick="S.tab='contacts';render()">👥<small>Люди</small></button><button class="tab ${S.tab==='profile'?'active':''}" onclick="S.tab='profile';render()">👤<small>Профиль</small></button><button class="tab ${S.tab==='notes'?'active':''}" onclick="S.tab='notes';render()">📝<small>Заметки</small></button><button class="tab ${S.tab==='links'?'active':''}" onclick="S.tab='links';render()">🔗<small>Ссылки</small></button><button class="tab ${S.tab==='downloads'?'active':''}" onclick="S.tab='downloads';render()">📁<small>Файлы</small></button><button class="tab ${S.tab==='settings'?'active':''}" onclick="S.tab='settings';render()">⚙<small>Опции</small></button><button class="tab bottom" id="closeAppBtn">⏻<small>Закрыть</small></button><button class="tab" onclick="logout()">⇥<small>Аккаунт</small></button></div>${renderLeft()}<main class="main">${renderCenter()}</main><aside class="side">${renderSide()}</aside></div>`;bind(); if(S.active)setTimeout(()=>scrollChatBottom(false),0)}
...
async function checkUpdates(){
  try{
    ...
    if(r?.dev)return modal(`<h2>Автообновление</h2><div class=fileCard>Текущая версия: ${h(r.current||'dev')}</div><div class=muted>${h(r.message||'Автообновления работают только в установленной версии.')}</div>`);
    if(r?.error)return toast('Проверка обновлений: '+r.error);
    toast('Проверка обновлений запущена');
  }catch(e){toast('Проверка обновлений не удалась: '+e.message)}
}
function showUpdateModal(data={}){
  modal(`<h2>Доступно обновление NightVault ${h(data.version||'')}</h2><div class=fileCard>Текущая версия: ${h(data.current||'')}</div><div class=muted>Нажми «Обновить» — приложение скачает новую версию, закроется и откроется уже обновленным.</div><div id=updateProgress class=updateProgress style=display:none><div></div></div><button class=btn onclick="downloadNightVaultUpdate()">Обновить</button><button class='btn ghost' onclick='closeModal()'>Позже</button>`)
}
...
function showUpdateReadyModal(data={}){
  closeModal();
  modal(`<h2>Обновление скачано</h2><div class=fileCard>Версия ${h(data.version||'новая')} готова к установке.</div><div class=muted>NightVault закроется и через несколько секунд откроется уже обновленным.</div><button class=btn onclick="installNightVaultUpdate()">Перезапустить и установить</button><button class='btn ghost' onclick='closeModal()'>Позже</button>`)
}
function showChangelogModal(data={}){
  const changes=(data.changes||[]).map(x=>`<li>${h(x)}</li>`).join('');
  setTimeout(()=>modal(`<h2>${h(data.title||('Что нового в NightVault '+(data.version||'')))}</h2><div class=fileCard><b>Версия ${h(data.version||'')}</b></div><ul class=changeList>${changes||'<li>Улучшена стабильность приложения.</li>'}</ul><button class=btn onclick='closeModal()'>Понятно</button>`),700);
}
function bindUpdaterEvents(){
  try{nv.onUpdateAvailable(showUpdateModal);nv.onUpdateProgress(p=>{let bar=$('#updateProgress div');if(bar)bar.style.width=(p.percent||0)+'%'});nv.onUpdateDownloaded(showUpdateReadyModal);nv.onUpdateError(e=>toast('Ошибка обновления: '+(e.message||e)));nv.onChangelog(showChangelogModal)}catch{}
}
...
function bind(){
  $('#winClose')?.addEventListener('click',()=>nv.close()); $('#winMin')?.addEventListener('click',()=>nv.minimize()); $('#winFull')?.addEventListener('click',()=>nv.toggleFull()); $('#closeAppBtn')?.addEventListener('click',()=>nv.close());
  let q=$('#q'); if(q)q.oninput=e=>{S.q=e.target.value;render()};
...
function modal(x){document.body.insertAdjacentHTML('beforeend',`<div class=modalWrap onclick="closeModal()"><div class=modal onclick="event.stopPropagation()">${x}</div></div>`); $$('.modal [id^=rep_]').forEach(el=>loadRepLabel(el.id.replace('rep_','')))}function closeModal(){$('.modalWrap')?.remove()}
...
async function deleteChat(){document.querySelector('.ctx')?.remove();if(!S.active)return;if(!confirm('Удалить чат из списка?'))return;try{await api('/chats/'+S.active+'/delete',{method:'POST',body:'{}'});S.active=null;await loadChats(false);render()}catch(e){toast('Чат не удалён: '+e.message)}}
...
try{nv.onWindowState?.(data=>{S.fullscreen=!!data.fullscreen;if(S.user)render()})}catch{}
