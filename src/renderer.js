...
const defaultState=()=>({
  ...
  selected:new Set(), editId:null, micId:localStorage.nvMicId||'', accounts:JSON.parse(localStorage.nvAccounts||'[]'), chatBg:localStorage.nvChatBg||'particles', fontSize:Number(localStorage.nvFontSize||15), fontFamily:localStorage.nvFontFamily||'system', notes:localStorage.nvNotes||'', appVersion:localStorage.nvAppVersion||'...'
});
let S=defaultState();
let sock=null;
...
async function loadAppVersion(){
  try{
    ...
    if(v){S.appVersion=v;localStorage.nvAppVersion=v;}
  }catch{S.appVersion=S.appVersion||'dev'}
}
...
function bindUpdaterEvents(){
  try{nv.onUpdateAvailable(showUpdateModal);nv.onUpdateProgress(p=>{let bar=$('#updateProgress div');if(bar)bar.style.width=(p.percent||0)+'%'});nv.onUpdateDownloaded(showUpdateReadyModal);nv.onUpdateError(e=>modal(`<h2>Ошибка обновления</h2><div class=muted>${h(e.message||e)}</div><button class=btn onclick='closeModal()'>Понятно</button>`));nv.onUpdateStatus(s=>{if(s?.status==='not-available')toast('Установлена последняя версия: '+(s.version||S.appVersion||''))});nv.onChangelog(showChangelogModal)}catch{}
}
