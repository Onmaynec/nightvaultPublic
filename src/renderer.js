...
const nv = window.nv || {
  ...
  getVersion:async()=> '0.9.8', checkUpdates:async()=>({dev:true}), downloadUpdate:async()=>({ok:false}), installUpdate:async()=>({ok:false}),
...
const defaultState=()=>({
  ...
  appVersion:localStorage.nvAppVersion||'0.9.8', theme:localStorage.nvTheme||'crimson', accent:localStorage.nvAccent||'#e11b2f', blur:localStorage.nvBlur==='1',
...
async function loadAppVersion(){
  try{
    ...
    if(v&&v!=='dev'){S.appVersion=v;localStorage.nvAppVersion=v;} else {S.appVersion=localStorage.nvAppVersion||S.appVersion||'0.9.8';}
  }catch{S.appVersion=S.appVersion||localStorage.nvAppVersion||'0.9.8'}
}
...
function render(){if(!S.user)return renderAuth();applyVisualPrefs();app.innerHTML=titlebar()+`<div class="shell ${S.fullscreen?'fullscreen':'windowed'}"><div class="tabs"><button class="tab ${S.tab==='chats'?'active':''}" onclick="S.tab='chats';render()">💬<small>Чаты</small>${S.chats.reduce((a,c)=>a+(c.unread||0),0)?'<span class=count>'+S.chats.reduce((a,c)=>a+(c.unread||0),0)+'</span>':''}</button><button class="tab ${S.tab==='contacts'?'active':''}" onclick="S.tab='contacts';render()">👥<small>Люди</small></button><button class="tab ${S.tab==='profile'?'active':''}" onclick="S.tab='profile';render()">👤<small>Профиль</small></button><button class="tab ${S.tab==='notes'?'active':''}" onclick="S.tab='notes';render()">📝<small>Заметки</small></button><button class="tab ${S.tab==='links'?'active':''}" onclick="S.tab='links';render()">🔗<small>Ссылки</small></button><button class="tab ${S.tab==='downloads'?'active':''}" onclick="S.tab='downloads';render()">📁<small>Файлы</small></button><button class="tab ${S.tab==='settings'?'active':''}" onclick="S.tab='settings';render()">⚙<small>Опции</small></button><button class="tab bottom" id="minAppBtn">—<small>Свернуть</small></button><button class="tab" id="closeAppBtn">⏻<small>Закрыть</small></button><button class="tab" onclick="logout()">⇥<small>Аккаунт</small></button></div>${renderLeft()}<main class="main">${renderCenter()}</main><aside class="side">${renderSide()}</aside></div>`;bind();}
